...
const ConnexionManager = ({ open }) => {
  const { reader, connexionBadge, pending } = useSelector((state) => ({
  ...
  const renderContent = React.useCallback(
    (connexionBadge) => {
      ...
      return reader ? (
        <BadgeConnexionModal connexionBadge={connexionBadge} />
      ) : (
        <SimpleConnexionModal />
      );
    },
    [pending, reader],
  );
  ...
  return (
    <React.Fragment>
      <div>
        ...
        {connexionBadge ? (
          <React.Fragment>
            <div>Pin: {Array(pin.length + 1).join("*")}</div>
            <div className="modal-pad">
              ...
              <Button color="red" onClick={() => setPin("")}>
                Supprimer Pin
              </Button>
            </div>
          </React.Fragment>
