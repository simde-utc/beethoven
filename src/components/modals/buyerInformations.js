...
import "./css/modals-inspicteur.scss";
import {
...
const BuyerInformationsModal = () => {
  ...
  const closeModal = React.useCallback(
    () => dispatch(buyerInformationsAPI.resetCurrent()),
    [dispatch],
  );
  ...
  const cancelPurchase = React.useCallback(
    (pur_id) => {
      if (user) {
        ...
        data["last_purchases"] = data["last_purchases"].filter(
          (pur) => String(pur.pur_id) !== String(pur_id),
        );
        dispatch(
          buyerInformationsAPI.updateOne(
            null,
            { pur_id },
  ...
  return (
    <Modal open={Boolean(user)} centered={false} size="tiny">
      <Modal.Header className="modal-header">
        {user
          ? `${user.getFirstname()} ${user.getLastname()} (${user.getUsername()}) - Solde: ${user
              .getSolde()
              .toFixed(2)}€`
          : null}
      </Modal.Header>
      <Modal.Content className="modal-container">
              ...
              return (
                <div className="buyer-row" key={purchase.getKey()}>
                  ...
                  <span className="cell name">
                    {purchase.getArticle().getName()}
                  </span>
                  <span className="cell price">
                    {purchase.getPrice().toFixed(2)}€
                  </span>
                  <span
        ...
        <Button color="red" fluid onClick={closeModal}>
          Fermer
        </Button>
      </Modal.Content>
    </Modal>
...
const handlePurchases = (user, articles) => {
  ...
  return user
    .getLastPurchases()
    .filter((pur) => pur.getQte() >= 0 && !pur.getRemoved())
    .map((purchase) => {
      purchase.setArticle(
        articles.find(
          (article) => String(article.getKey()) === String(purchase.getObj()),
        ),
      );
      return purchase;
    });
